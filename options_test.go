...
func TestBuildPebbleOptionsProfiles(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			require.Empty(t, opts.KeySchema)
			require.Empty(t, opts.KeySchemas)
			prepared, registry, err := productionPebbleOptions(opts, false)
			require.NoError(t, err)
			require.Equal(t, registry.active, prepared.KeySchema)
			require.Contains(t, prepared.KeySchemas, prepared.KeySchema)
