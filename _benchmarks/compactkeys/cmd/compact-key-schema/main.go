...
func main() {
	output := flag.String("output", "compact-key-schema-results", "directory for manifests and summary.csv")
	workRoot := flag.String("work-root", "", "parent directory for temporary benchmark databases")
	rows := flag.Int("rows", 10_000, "logical rows per run")
	seed := flag.Int64("seed", 20260806, "deterministic dataset seed")
	...
	dataset, err := compactkeys.Generate(compactkeys.RepresentativeDatasetSpec(*seed, *rows))
	if err != nil {
		fatalf("generate dataset: %v", err)
	}
	candidates := compactkeys.FullKeyCandidates(bond.MediumPerformance)

	var manifests []compactkeys.RunManifest
	for _, candidate := range candidates {
		for warmup := 1; warmup <= *warmups; warmup++ {
			fmt.Printf("warmup candidate=%s run=%d/%d\n", candidate.Name, warmup, *warmups)
		...
		for repetition := 1; repetition <= *repetitions; repetition++ {
			fmt.Printf("retain candidate=%s run=%d/%d\n", candidate.Name, repetition, *repetitions)
...
func run(
	...
	runRoot, err := os.MkdirTemp(tempParent, "bond-full-key-schema-")
	if err != nil {
...
func writeSummary(path string, manifests []compactkeys.RunManifest) error {
	...
	rows := [][]string{{
		"candidate", "writer_schema", "bundle_size", "run", "dataset_digest", "database_bytes",
		"sst_physical_bytes", "schema_physical_bytes", "property_raw_key_bytes", "property_data_bytes",
		"property_index_uncompressed_bytes", "property_filter_bytes", "key_schema_files", "compaction_ns",
		"compaction_cpu_ns", "write_amplification", "hit_p95_ns", "miss_p95_ns", "registered_readers",
	}}
	for _, manifest := range manifests {
		...
		rows = append(rows, []string{
			...
			strconv.Itoa(manifest.Schema.ActiveBundleSize),
			...
			strconv.FormatUint(result.AfterCompaction.SST.KeySchemaBytes[manifest.ActiveKeySchema], 10),
			...
			strconv.FormatUint(result.AfterCompaction.SST.PropertyFilterBytes, 10),
			strconv.Itoa(result.AfterCompaction.SST.KeySchemas[manifest.ActiveKeySchema]),
