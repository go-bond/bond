...
func main() {
	output := flag.String("output", "compact-key-results", "directory for manifests and summary.csv")
	...
	candidates := compactkeys.BaselineCandidates(bond.MediumPerformance)
...
func run(repoRoot, tempParent string, engine compactkeys.EngineSpec, dataset compactkeys.Dataset, runSpec compactkeys.RunSpec) (compactkeys.RunManifest, error) {
	runRoot, err := os.MkdirTemp(tempParent, "bond-compact-key-")
	if err != nil {
...
func writeSummary(path string, manifests []compactkeys.RunManifest) error {
	...
	rows := [][]string{{
		"candidate", "run", "dataset_digest", "database_bytes", "sst_physical_bytes", "property_data_bytes", "property_index_uncompressed_bytes", "property_filter_bytes",
		"filter_false_positive_rate", "compaction_ns", "compaction_cpu_ns", "write_amplification", "hit_p95_ns", "miss_p95_ns",
	}}
	for _, manifest := range manifests {
		...
		rows = append(rows, []string{
			...
			strconv.FormatFloat(result.Filter.FalsePositiveRate, 'g', -1, 64),
